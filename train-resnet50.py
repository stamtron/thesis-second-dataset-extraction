...
train_loader = get_loader(1, 4, end_idx, class_image_paths, train_transform, tensor_transform, False, True)
df = pd.read_csv('./important_csvs/events_with_number_of_frames_stratified.csv')
df = get_df(df, 16, True)
class_image_paths, end_idx = get_indices(df)
valid_loader = get_loader(1, 4, end_idx, class_image_paths, valid_transform, tensor_transform, False, True)
...
train_model(dataloaders, device, resnet, criterion, optimizer, scheduler, num_epochs=6)