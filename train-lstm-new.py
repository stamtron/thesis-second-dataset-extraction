...
seq_length = 25
indices, labels = get_final_indices_train(idx_label, end_idx, 'overlap', skip_frames=15, set_step=12, seq_length=25, per_label=True)
...
indices, labels = get_final_indices_valid(idx_label, end_idx, 'overlap', set_step=12, seq_length=25, per_label=True)
valid_loader, valid_dataset = get_loader_new(25, bs, indices, class_image_paths, valid_temp_transform,
                                             valid_spat_transform, tensor_transform, True, False, True, 1)

df_test = get_df(df, 50, False, False, True)
class_image_paths, end_idx, idx_label = get_indices(df_test, root_dir)
indices, labels = get_final_indices_valid(idx_label, end_idx, 'overlap', set_step=12, seq_length=25, per_label=True)
test_loader, test_dataset = get_loader_new(25, bs, indices, class_image_paths, valid_temp_transform, 
                                            valid_spat_transform, tensor_transform, True, False, True, 1)
...
load = False
if load:
    checkpoint = torch.load('/media/scratch/astamoulakatos/saved-lstm-models/secondnewagain/best-checkpoint-013epoch.pth')
...
device = torch.device('cuda')
writer = SummaryWriter('runs/ResNet2D_LSTM_small_block')
