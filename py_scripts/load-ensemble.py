...
checkpoint = torch.load('/media/hdd/astamoulakatos/save-model-3d/save_freezed_3.pth')
model.load_state_dict(checkpoint['state_dict'])
print('loading pretrained freezed model!')

model.module.fc = nn.Sequential(*list(model.module.fc.children())[:-4])

cnn_encoder, rnn_decoder = load_model(bs=6)
...
rnn_decoder.load_state_dict(torch.load('/media/hdd/astamoulakatos/save-model-lstm/rnn_decoder_epoch_freezed_yo_1.pth'), strict=False)
cnn_encoder.load_state_dict(torch.load('/media/hdd/astamoulakatos/save-model-lstm/cnn_encoder_epoch_freezed_yo_1.pth'),strict=False)
...
train_loader = load_data(df_train, 6, 16)
valid_loader = load_data(df_valid, 6, 16)
...
checkpoint = torch.load('/media/hdd/astamoulakatos/save-ensemble-model/save_ensemble_1.pth')
model.load_state_dict(checkpoint['state_dict'])
print('loading pretrained freezed model!')

epochs = 2
criterion = nn.BCEWithLogitsLoss()
optimizer = torch.optim.SGD(ensemble_model.parameters(), lr=0.01, momentum=0.9)
