...
cnn_encoder, rnn_decoder = load_model(bs=32)
...
rnn_decoder.load_state_dict(torch.load('/media/scratch/astamoulakatos/save-model-lstm/rnn_decoder_epoch_freezed_yo_1.pth'), strict=False)

cnn_encoder.load_state_dict(torch.load('/media/scratch/astamoulakatos/save-model-lstm/cnn_encoder_epoch_freezed_yo_1.pth'),strict=False)
...
optimizer.load_state_dict(torch.load('/media/scratch/astamoulakatos/save-model-lstm/optimizer_epoch_freezed_yo_1.pth'))
...
for epoch in trange(epochs, desc="Epochs"):    
    ...
    torch.save(cnn_encoder.state_dict(), os.path.join(save_model_path, 'cnn_encoder_epoch_unfreezed_{}.pth'.format(epoch)))  # save spatial_encoder
    torch.save(rnn_decoder.state_dict(), os.path.join(save_model_path, 'rnn_decoder_epoch_unfreezed_{}.pth'.format(epoch)))  # save motion_encoder
    torch.save(optimizer.state_dict(), os.path.join(save_model_path, 'optimizer_epoch_unfreezed_{}.pth'.format(epoch)))      # save optimizer
